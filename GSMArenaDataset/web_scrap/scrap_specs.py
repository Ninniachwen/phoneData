...
def get_specs(html,specs):
    ...
    # Sound
    loudspeaker1 = soup.findAll('a',text='Loudspeaker')
    loudspeaker1 = None if (loudspeaker1 is None) or (len(loudspeaker1)==0) else loudspeaker1[0].parent.parent.find('td',class_='nfo')
    ...
    popularity.append(None) if (popularity1 is None) or (len(popularity1)==0) else popularity.append(popularity1[0].get_text())
...
def remove_line_breaks_from_dict(specs):
    """
    Removes line breaks from all string values in a dictionary.
    """
    for key, value in specs.items():
        if isinstance(value, str):  # Check if the value is a string
            specs[key] = value.replace("\n", "_").replace("\r", "_").strip()
    return specs
...
for ip_address in ip_addresses:
    ...
    while i<len(links):
        try:
            ...
            #specs = remove_line_breaks_from_dict(specs) # Preprocess dictionary to remove line breaks
            i=i+1
            print(f"{i} links are done.")
            if i%1==0:
                if specs['Name']==[None]:
                    print(i, f" items imported. please restart with i={i-1}")
                    quit(1)
                temp = pd.read_csv(filepath,index_col=0)
                df = pd.concat([temp,pd.DataFrame(specs)],ignore_index=True)
                cleaned_df = df.applymap(lambda x: x.replace("\n", "_").replace("\r", "_").replace("‑", "-").replace(" ", " ").replace(" ", " ").replace("   ", " ").replace("  ", " ") if isinstance(x, str) else x)
    ...
    if response.status_code == 429:
        if ip_address == ip_addresses[-1]:
            print(">>>>>>>>   ran out of IP adresses. please regenerate   <<<<<<<<")
        continue
